...
const SearchPage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto my-10 ">
      <div className="flex flex-col md:flex-row justify-between gap-10 mx-5">
        <FilterPage />
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <Button onClick={()=> searchRestaurant(params.text!,searchQuery,appliedFilter)} className="bg-red hover:bg-hoverRed">Search</Button>
          </div>
          {/* Searched items displayed here  */}
          <div>
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-3 my-3">
              <h1 className="font-medium text-xl">  ({searchedRestaurant?.data.length}) Search result found</h1>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {
                loading ? <SearchPageSkeleton/> :(
                  ...
                  searchedRestaurant?.data.map((restaurant:Restaurant, idx:number)=>(
                      <Card key={restaurant._id} className="bg-white dark:bg-gray-800 shadow-lg rounded-xl overflow-hidden hover:shadow-2xl transition-shadow duration-300">
